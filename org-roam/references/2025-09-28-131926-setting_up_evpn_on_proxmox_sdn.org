...
Ethernet VPN (EVPN) with VXLAN is a common way to build scalable L2/L3 overlays with a BGP-based control plane. This note walks through setting up EVPN on [[id:77bd7428-f1ee-4306-8d5a-62f38134dfc5][Proxmox VE]] SDN to create a flexible, resilient overlay fabric.
...
:END:
Ethernet VPN (EVPN) is a control-plane technology that carries Layer 2 Ethernet reachability using [[id:e7b30b16-d942-4c41-ba19-14245c12a572][BGP]](Border Gateway Protocol). It distributes MAC (and optionally IP) reachability efficiently, reducing flooding and improving scalability. EVPN also supports features such as active/active multihoming, MAC mobility, and ARP/ND suppression, which makes it a strong fit for multi-tenant overlays.
...
+ [[id:fd557d1a-33a5-457b-962d-ae81d7da0644][FRRouting]]: Install *frr-pythontools* on all nodes (used by SDN for BGP/EVPN).
+ Network configuration: Ensure your underlay networking is working and *ifupdown2* is installed.
+ [[id:e495805e-5687-4c21-8fbe-98c683b2fc18][DHCP]]/IPAM: PVE’s built-in IPAM/DHCP relies on [[id:c6e817cc-0f08-4dd6-a69c-67384d83fd68][dnsmasq]], which currently does not support [[id:993d6751-aee5-4f90-9a86-8ca207739991][EVPN]]. For EVPN networks, use an external IPAM/DHCP source such as [[id:3fb5adf6-b8c8-43cd-9726-c1d126458e4e][NetBox: The Network Source of Truth]] with [[id:a8fd1452-8453-4aa4-89bc-37c8d22d12c1][Kea DHCP]]; see [[id:af704067-3d10-4745-93c3-297e26fb91d0][Enable DHCP Controller]].
+ [[id:7bab7928-237d-4784-a42f-b85ef6874b9b][Domain Name Service (DNS)]] (optional): If you want automatic DNS registration for guests, configure a DNS integration (for example [[id:d15d64c4-08d3-48a4-88b1-184a56710400][PowerDNS]]). This is typically paired with external IPAM.

* Step-by-Step Guide
The steps below are based on my [[id:d959bbdf-e766-4d5f-a0c5-486e68b5b4e1][Data Center Testbed]].

** ASNs and roles
+ [[id:fe7713cb-166a-46c1-8a1d-8ceca7e61691][VyOS]] edge (all routers): ASN 65001
...
** VyOS [[id:e7b30b16-d942-4c41-ba19-14245c12a572][BGP]] configs (on three gateways; same neighbors; only `router-id` differs)
*** R0
#+begin_src bash
  configure
...
If you want automatic IP assignment and DNS registration (for example with [[id:d15d64c4-08d3-48a4-88b1-184a56710400][PowerDNS]]), you need an IPAM/DHCP workflow.
However, PVE’s built-in DHCP controller is [[id:c6e817cc-0f08-4dd6-a69c-67384d83fd68][dnsmasq]]-based, and [[id:c6e817cc-0f08-4dd6-a69c-67384d83fd68][dnsmasq]] does not support [[id:993d6751-aee5-4f90-9a86-8ca207739991][EVPN]].

1. *(Optional)* Install [[id:c6e817cc-0f08-4dd6-a69c-67384d83fd68][dnsmasq]] on all Proxmox nodes (needed for non-EVPN SDN DHCP use-cases):
   #+begin_src bash
   apt install dnsmasq
...
2. For EVPN VNets, set up external IPAM/DHCP (recommended: [[id:3fb5adf6-b8c8-43cd-9726-c1d126458e4e][NetBox]]; alternative: [[id:30e29519-ed12-4a53-ae77-3172adea7d49][phpIPAM]]) and integrate it via Proxmox SDN.
...
+ IPAM: `pve` (local IPAM) or `nnetbox` (if you integrated NetBox)
...
+ Gateway: 10.60.10.1 (Proxmox will install this as an anycast VRF gateway on the EVPN fabric, and it will advertise 10.60.10.0/24 to VyOS via BGP if “Advertise Subnets” is enabled in the zone)
+ SNAT: enable only if you want the Proxmox exit nodes to NAT this subnet; if VyOS is your Internet gateway doing NAT, leave this unchecked and configure [[id:747d6a79-c275-4ef4-b98e-c1f651ae668a][Internet access / NAT for EVPN subnet]] on VyOS to avoid double NAT
